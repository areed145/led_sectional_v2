...
def ledMetars(strip, df, stat, i):
    #strip.setPixelColorRGB(0, int(colorMapStat(stat)[0] * 255), int(colorMapStat(stat)[1] * 255), int(colorMapStat(stat)[2] * 255))
    #print('UPDATE: ' + str(i) + ', LED: ' + '0' + ', SID: ' + str(stat) + ', DATA: ' + 'status' + ', COLOR: ' + str([int(colorMapStat$
    if stat == float(1):
        for led in df.index:
            r = df['R'][led]
            g = df['G'][led]
            ...
            #print('UPDATE: ' + str(i) + ', LED: ' + str(led) + ', SID: ' + str(station) + ', DATA: ' + str(data) + ', COLOR: ['+str(r$
    #strip.show()
